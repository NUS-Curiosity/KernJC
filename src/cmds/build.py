...
def handler(args):
    ...
    if args.kernel is None:
        ...
        if kernel_version is None:
            ...
            return
        end_time = time.time()
        logger.info(f"Version Identification time: {end_time - start_time} seconds")
    ...
    end_time = time.time()
    logger.info(f"Environment build time: {end_time - start_time} seconds with {opt} option for {cve}")
